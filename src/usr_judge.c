...
void game_settle_landing(Game *g)  /* 落点处理（规范 9） */
{
    ...
    if (g->players[g->current_index].position == TOOL_POS)
        //等待一个玩家进入道具屋的函数
    if (g->players[g->current_index].position == JAIL_POS)
        //等待一个玩家进入监狱的函数
    if (g->players[g->current_index].position == GIFT_POS)
        //等待一个玩家进入礼品屋的函数
    if (g->players[g->current_index].position > 63 && g->players[g->current_index].position < 70)
        //等待一个玩家进入矿地的函数
    else
        //等待一个玩家进入空地的函数
}
