...
{
    if (initial_fund < MANUAL_INITIAL_FUND_MIN || initial_fund > MANUAL_INITIAL_FUND_MAX) {
        set_error("初始资金必须在 %d~%d 之间", MANUAL_INITIAL_FUND_MIN, MANUAL_INITIAL_FUND_MAX);
...
/* TODO(下一迭代)：ROLL/STEP 共用逐格移动逻辑（规范 8.1），
   每进入一格检查 BLOCK/BOMB（规范 4.2）。 */
int game_roll(Game *g)
{
    (void)g;
    set_error("TODO: game_roll 尚未实现（规范 8.1 ROLL）");
    return RC_INTERNAL;
}
...
{
    (void)g; (void)steps;
    set_error("TODO: game_step 尚未实现（规范 8.1 STEP）");
    return RC_INTERNAL;
}
...
{
    (void)g; (void)offset;
    set_error("TODO: game_block 尚未实现（规范 8.3 BLOCK）");
    return RC_INTERNAL;
}
...
{
    (void)g; (void)offset;
    set_error("TODO: game_bomb 尚未实现（规范 8.3 BOMB）");
    return RC_INTERNAL;
}
...
{
    (void)g;
    set_error("TODO: game_robot 尚未实现（规范 8.4 ROBOT）");
    return RC_INTERNAL;
}

int game_answer(Game *g, const char *value)
{
    if (g != NULL && g->prompt == PROMPT_TOOL_SHOP) {
        char message[1024];
        int rc = tool_shop_answer(g, value, message, sizeof(message));
        if (message[0] != '\0') {
            printf("%s\n", message);
        }
        return rc;
...
/* TODO(下一迭代)：逐格移动与途中道具（规范 4.2）：
   position = (position+1)%70，检查 BLOCK 停止/BOMB 送医院，
   移动完成后调用 game_settle_landing。 */
int game_move_to(Game *g, int32_t steps)
{
    (void)g; (void)steps;
    set_error("TODO: game_move_to 尚未实现（规范 4.2 途中道具）");
    return RC_INTERNAL;
}

/* TODO(下一迭代)：落点处理（规范 9）：
   无人地产→BUY 提示；自己地产 level<3→UPGRADE 提示；level=3→结束回合；
   他人地产→财神/租金/破产（规范 9.1）；START 无效果；HOSPITAL 经过不住院；
   JAIL→入狱 2 轮；MINE→加点数；TOOL_SHOP/GIFT_SHOP→对应提示；
   MAGIC_HOUSE→项目统一配置。 */
void game_settle_landing(Game *g, int32_t position)
{
    ...
    case CELL_MINE:
        player->credit += g->cells[position].mine_points;
        break;
    case CELL_JAIL:
